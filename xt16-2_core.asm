...
read_hard_disk_with_gate:                   ;从硬盘读取一个逻辑扇区
         ...
         mov ax,[esp+0x14]                  ;获取调用者的CS
         arpl [esp+0x1c],ax                 ;将数据段选择子调整到真实的请求特权级别
         mov ds,[esp+0x1c]                  ;用真实的段选择子加载段寄存器DS

         mov eax,[esp+0x20]                 ;从栈中取得逻辑扇区号
         mov ebx,[esp+0x18]                 ;从栈中取得缓冲区在段内的偏移量
  ...
  with_gate.waits:
         ...
         and al,0x88
         cmp al,0x08
         jnz with_gate.waits                         ;不忙，且硬盘已准备好数据传输 
...
start:
  ...
  .b3:
         ...
         or cx,1_11_0_1100_000_00000B     ;特权级3的调用门(3以上的特权级才
                                          ;允许访问)
